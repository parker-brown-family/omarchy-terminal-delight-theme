...
#endif
// ---- terminal-delight per-window curved glass ----
// Barrel polynomial f = 1 + k1*r^2 + k2*r^4 in window-local UV space.
// k1/k2 = 0.14/0.06 -- the pane constants from terminal-delight
// (web-warp-lab approach 03; asserted in app/src/warp.rs).
...
void main() {
#if USE_ROUNDING && !USE_MOTION_BLUR
    // terminal-delight: bow the window content like curved glass
    vec2  td_c    = v_texcoord - 0.5;
    float td_r2   = dot(td_c, td_c);
    vec2  td_uv   = 0.5 + td_c * (1.0 + TD_K1 * td_r2 + TD_K2 * td_r2 * td_r2);
    vec2  td_e    = min(td_uv, 1.0 - td_uv);
    float td_edge = smoothstep(0.0, 0.004, min(td_e.x, td_e.y));
...
#else
    vec4 pixColor = vec4(texture(tex, td_uv).rgb, 1.0);
#endif
#if USE_BLUR_MATTE
